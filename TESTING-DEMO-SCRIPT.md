...
- **Live Results goes to zero the moment an election ends -- the final number does not vanish, it moves.** End of Voting resets the *live* counter to zero immediately (so the next election of that type starts from a clean slate), but the real final tally is saved permanently to Election History in that same click, before the reset happens. If Live Results looks blank right after ending an election, that's expected -- there's no election running any more for it to show. Look at Election History (or Download Report, which falls back to the same saved figure) for the number that actually matters; it's never lost. *(Verified 2026-09-24 against a live server: 3 votes cast, Live Results showed 3, End of Voting clicked, Live Results immediately showed 0, Election History showed 3.)*
...
- **2026-09-24 — a claim that votes are not actually reset to zero on End of Voting was checked, not assumed.** Reported against the live confirm dialog's own wording ("resets votes to zero"), with the belief that it was wrong and that votes only reset when the next election of that type starts. Tested directly against a running server: cast 3 votes, confirmed Live Results showed 3, called End of Voting, confirmed Live Results immediately showed 0 and Election History showed 3. The reset-to-zero behavior is correct and deliberate (see Part 3's new "Live Results goes to zero" entry for why) and was left unchanged. What *did* change, by direct request, is the confirm dialog's wording -- cut from three sentences to one: **"End of Voting for \"[name]\"?\n\nThis saves the final result to Election History, and closes the poll."** The previous version's "Officer codes and their allotments are kept for the next election" and "This cannot be undone" sentences were both part of that cut; worth a second look if either of those turns out to be missed.
