...
> **You will not need "🧹 Clear Test Votes" anywhere in this demo.** It only exists for clearing stray test votes cast without ever using Start the Election Process, and only appears at all in the gap between elections — it's tucked under the "no election in progress" box, not sitting next to the report buttons. **End the Election Process** is the correct way to finish any election, every time. (See Part 3.)
...
- **Clear Test Votes (the API is still `/api/poll/reset`) is blocked while the poll is open** (403, "Close the poll before resetting"). *(Fixed 2026-09-24: previously only the admin UI disabled the button, with no server-side guard — a direct API call could have reset live votes out from under a voter mid-ballot.)*
- **It is blocked for as long as an election is in progress — including during a Pause Polling break**, not just while polling is actively open (403, "An election is currently in progress. Use 'End the Election Process' to finish it"). *(Fixed 2026-09-24: Pause Polling stops new votes but does not end the election, so the first guard above alone still left a window — pause for a lunch break, then this would silently wipe that election's votes while it was still under way. Verified: blocked immediately after Pause Polling, works again immediately after End the Election Process, on both the button and the API directly.)*
- **The button itself no longer even appears during an election** (2026-09-24) — previously it sat, prominent and orange, in "Records & Reports" next to the Download Report buttons, labelled "Reset Poll (Clear All Votes)", regardless of whether it could actually be used. It's now a small secondary "🧹 Clear Test Votes" button inside the "no election in progress" box, and doesn't exist in the page at all once an election has started.
...
- **"Clear Test Votes" only exists on screen at all when no election is in progress.** It used to be a permanent, prominent button (named "Reset Poll") sitting next to the report buttons, always visible whether or not it could be used. Its only real job was ever clearing stray test votes cast without going through Start the Election Process -- "Start the Election Process" itself already wipes whatever votes exist for the type it starts. It's now tucked into the "no election in progress" box, where that job is actually relevant, and it's not in the page at all once an election starts (a Pause Polling break included). Use **End the Election Process** to finish an election properly; it saves the final result and clears the votes together.
...
- **2026-09-24 — the button itself renamed and relocated.** Once it was clear its only remaining job is clearing stray test votes cast without ever starting a formal election -- "Start the Election Process" already wipes whatever votes exist for the type it starts, on its own -- it no longer made sense as a permanent, prominent orange button in "Records & Reports" sitting right next to the Download Report buttons. It's now a small "🧹 Clear Test Votes" button inside the "no election in progress" box, and it doesn't render in the page at all while an election is running.
