...
23. **End of Voting.** Dashboard → **End of Voting**. This saves the final result to Election History and closes the poll. The banner disappears. Officer codes and their teacher allotments are kept.
24. **Confirm nothing was lost — and nothing went blank either.** Polling Officer Codes tab — every code and every officer name is still there, and any booth that had been closed is usable again. Live Results tab — still shows the final numbers, exactly as they were the moment voting ended (the live count does **not** reset here; only Start the Voting Process resets it, when the next School election actually begins). Election History — the School election's final record is there, and did **not** get duplicated by the checkpoint you saved in step 19.
...
33. **Reports survive.** With nothing running at all, **Download Dwarka Report** / **Download AN Report** still work and show the House election's final result — the most recently closed election. They never go permanently dark.
34. **Election History.** Show **View/Print Dwarka** and **View/Print AN** on a past entry. Point out that the vote totals on these match what was counted on the day. Show renaming an entry, and the confirmation prompt on deleting one.
35. **Officer turnout.** On an Election History report, untick **"Include polling officer turnout"** for a results-only copy to hand out; it's ticked by default.
36. **Activity Log, by election name.** Open the **Activity Log** tab. Both elections you just ran show up as named, clickable entries under "Elections" — click **House 3 try** (or whatever you named it) and the log narrows immediately to just that election's actions, no dropdown or Search press needed. Click **All Elections** to go back to everything.
    **Say:** *This log runs for the whole span of an election — start to end, including through any pause — and it's permanent. Nobody, including me, can edit or delete an entry in it.*
37. **Storage indicator.** Point at the green "Storage: OK" on the Dashboard once, so whoever is on duty would recognise the red warning banner if storage ever failed for real.
...
- **School posts lay out full-width, one post per row, same as House.** School candidate cards used to sit in a narrow multi-column grid (5 posts side by side); editing a name there could push the Save/Cancel buttons off the edge of the card, out of reach. Fixed 2026-09-25 by giving School the same full-width, one-post-per-row layout House already had, where this never happened. **This is a pure CSS/layout change and cannot be meaningfully checked by an automated test (no real browser layout in the test environment) — needs a real browser check:** open Manage Candidates → School, click Edit on any candidate, confirm the ✓ Save and ✕ Cancel buttons are both fully visible and clickable, at both a normal desktop width and a narrow/mobile width.
...
- **Admin can close a booth directly** from the Officer Codes tab (a new "Close" button next to Delete, shown on any open code) — no need to open a kiosk tab and enter the code there to do the officer's own "Close Polling at This Booth" on their behalf. *(Added 2026-09-25. Verified end to end: closes the code, a subsequent Activate Ballot with it is refused, Reopen makes it usable again.)*
- **Deleted code** → "incorrect officer code". *(Verified 2026-09-24.)* **A named-but-unused code can now be deleted** (revised 2026-09-25, by direct request — see the Part 3 entry below for why); the only remaining condition is a vote actually cast under it, which still blocks deletion regardless of naming. *(Verified end to end: a named, unvoted code deletes cleanly (204); a named code with one vote cast under it is still refused (409).)*
- **Codes survive an election.** End an election with a code allotted and its booth closed, start a new election of the same type — the code, officer name and allotment are all still there and the booth is usable again. *(Verified 2026-09-24.)* The code's *vote count* stays exactly as it was at End of Voting (it does **not** go to zero at close) -- it only resets the moment a new election of that type is actually started. Verified end to end: 3 votes cast, End of Voting, voteCount still 3; started a fresh election of the same type, voteCount then 0.
...
### Activity Log
- **Named elections are directly clickable.** Every past election appears as its own button under "Elections," labelled with the name given when it was started; clicking one filters the log to just that election in one click, no dropdown or Search press needed. *(Added 2026-09-24.)*
- **The recording window matches what's claimed: start-to-end, through a pause, not affected by it.** Logging begins the moment Start the Voting Process opens the poll, and continues right through any Pause Polling break -- it only stops at End of Voting. Setup work before an election starts (adding candidates, generating codes) is never logged. *(Behavior unchanged; verified consistent with this by design throughout the session's other changes.)*
- **A still-running election is marked.** Its button shows 🔴 while `status: running`; past elections don't.
...
- **There is no "Reset Poll" button any more.** It used to sit, prominent and orange, next to the report buttons in "Records & Reports" -- but the poll can only ever be opened through Start the Voting Process (which starts the election in the same click), so there was never a way to reach the state it was meant to fix. Use **End of Voting** to finish an election properly; it saves the final result and closes the poll -- it deliberately leaves the votes themselves untouched, see the "Live Results does NOT go blank" entry above.
- **Live Results does NOT go blank when an election ends -- it keeps showing the final numbers.** End of Voting saves the result to Election History and closes the poll, but deliberately does not touch the live vote count (2026-09-24, reversed from an earlier version of this app that did reset it here): a school's House/School election runs once a year, so there's no reason for the number everyone just watched fill in to vanish the moment it's over. Live Results, the Dashboard's own total, and each officer code's turnout all keep showing that election's final figures for as long as nobody has started a new election of that type. The live count only resets the moment **Start the Voting Process** is used for that type again -- that action, and only that action, wipes it back to zero. *(Verified 2026-09-24 against a live server: 3 votes cast, End of Voting, Live Results and officer code turnout both still showed 3; started a fresh election of the same type, both then showed 0. The earlier election's archived record in Election History was untouched throughout.)*
...
- **A named code with zero votes CAN now be deleted** (revised 2026-09-25, ELECTION-INTEGRITY-AND-TRUST.md item 3 addendum) — only a code that has actually cast a vote is permanent. This matches how codes are actually allotted now: a whole staff roster gets a code each, sent by WhatsApp, and it's routine for some teachers not to report for duty. Every naming and every deletion is still permanently logged either way, so that history is never actually lost.
...
- **2026-09-24 — "Refresh this Page" removed entirely (both copies).** By direct feedback: the previous entry above moved a second copy of this button to the Dashboard's title row rather than questioning whether it was needed at all, leaving two "Refresh this Page" buttons visible at once with no real difference between them. Neither is there any more -- the browser's own refresh does the same job, and given only one admin device can hold control at a time and every action on this screen already reloads its own data afterward, there was little this button did that a real page reload didn't. The "Screen Data" status card's hint text no longer references it either, and now simply ends at "...updates automatically every 3 seconds while the poll is open."
- **2026-09-24 — a claim that votes are not actually reset to zero on End of Voting was checked, not assumed -- and, on that first check, confirmed the reset was happening.** Reported against the live confirm dialog's own wording ("resets votes to zero"). Tested directly against a running server: cast 3 votes, confirmed Live Results showed 3, called End of Voting, confirmed Live Results immediately showed 0 and Election History showed 3 -- so at that point the reset genuinely was happening as the dialog said, and was left unchanged; only the dialog's wording was shortened, cut from three sentences to one: **"End of Voting for \"[name]\"?\n\nThis saves the final result to Election History, and closes the poll."** (The previous version's "Officer codes and their allotments are kept for the next election" and "This cannot be undone" sentences were both part of that cut.)
- **2026-09-24 — later the same day, told directly that the reset-at-close behavior itself was unwanted, and it was changed.** The reasoning: a School/House election runs once a year, so there's no reason the live count needs to go blank the instant voting ends, and it should instead stay frozen everywhere (Live Results, the Dashboard total, each officer code's turnout) for as long as nobody has started a new election of that type -- resetting only when **Start the Voting Process** genuinely begins a new one, which already happens on its own via that action's existing safety-net reset. Checked first that this was safe: votes live in their own Firestore subcollection, not the size-limited main document, so keeping a year's worth of votes around costs nothing; `startRecording`'s own reset already guarantees a clean slate regardless of how the previous election ended. Removed the vote-reset call from `closeRecording` (runService.ts) -- End of Voting now only archives and closes the poll. Verified end to end against a live server: 3 votes cast, End of Voting, Live Results and officer code turnout both still showed 3; started a fresh election of the same type, both then showed 0, and Election History's own record of the first election was untouched throughout.
- **2026-09-24 — Activity Log's elections are now directly clickable by name.** Requested against the existing "Election" dropdown-plus-Search-button combo: picking a specific election is the single most common thing to want from this tab, and shouldn't need opening a dropdown and pressing Search. Added a row of named buttons above the existing filter form -- one per past election (plus "All Elections"), with a 🔴 marker on one still running -- that filters the log to that election in a single click, via the same underlying state the dropdown already used, so combining a named election with other filters (branch, actor, code) in the form still works exactly as before. The dropdown itself was left in place rather than removed, since it still serves that combined-filter case; this is a shortcut alongside it, not a replacement. New test coverage: `AdminLandingPage.test.tsx`, confirming both past elections render as named buttons and that clicking one calls the search with only that `runId` set.
- **2026-09-25 — officer code deletion rule relaxed to match the real workflow, and admin can now close a code directly.** Two related requests:
  1. A named-but-unvoted code could not be deleted at all -- a deliberate rule from ELECTION-INTEGRITY-AND-TRUST.md item 3, originally specified by the Election Commissioner to close a specific fraud gap (naming a code, deciding not to use it, quietly deleting it before anyone reviews the roster). That rule no longer fits how codes are actually allotted: a full staff roster is loaded in at once, a code generated and named for every teacher on it, sent by WhatsApp, and it's routine for some not to report for duty -- their codes were always going to sit named-and-unused, indistinguishable from the case the rule was built to catch. Relaxed to the one condition that still matters: a code that has cast a vote is permanent; a named-but-unvoted one can now be deleted. The abuse case is still fully covered, since a code can't vote at all until it's named (so "used, then deleted before review" was never actually reachable once vote-count is checked), and every naming and deletion stays permanently logged either way. Documented as a revision, not silently reversed, in ELECTION-INTEGRITY-AND-TRUST.md item 3.
  2. The only way to close a booth used to be self-service, from a kiosk -- an admin who needed to close one (a teacher who couldn't reach a kiosk, say) had to open a kiosk tab and enter the code there themselves. Added a direct **Close** button next to Delete on any open code in the Officer Codes tab, calling a new admin route (`POST /officer-codes/:code/close`) that does the exact same thing as the kiosk's own "Close Polling at This Booth," just reachable from the admin screen. Also fixed the activation error a voter sees on a closed code, which used to say "closed by the polling officer" -- no longer accurate now that an admin can close one too, so it's generic now.
  Verified end to end against a live server: a named, unvoted, never-used code deletes cleanly (204); the same code with one vote cast under it is still refused (409); the new admin Close route stamps `closedAt` and a subsequent Activate Ballot with that code is refused, then Reopen restores it. New backend test coverage in `officerCodes.test.ts` (flipped two tests from "refuses" to "allows," added a `POST /:code/close` suite) and frontend coverage in `AdminLandingPage.test.tsx` (Close/Reopen buttons render correctly per code state, clicking Close calls the right API).
- **2026-09-24 — Activity Log tab description cut from four sentences to two**, by direct feedback that it "needs serious editing." Kept the permanence guarantee and the two how-to-use pointers; dropped the exact recording-window and setup-work-exclusion details, which are documentation of the system's behavior rather than instructions for using the screen -- they're still covered in this script's own Activity Log section above.
- **2026-09-25 — Manage Candidates: School posts moved to the same full-width, one-post-per-row layout House already used.** Reported directly: editing a School candidate's name could hide the Save/Cancel buttons. Root cause was CSS, not logic -- School posts sat in a narrow multi-column grid (`.results-grid`, `minmax(200px, 1fr)`, 5 posts side by side), while House posts were already laid out full-width, one at a time. `CandidateEditor`'s edit-mode row (photo + text input + Save + Cancel, all on one line, no wrap) fit fine at full width but ran past the edge of a ~200px grid cell, pushing the buttons out of reach. Gave School the same full-width layout House already had; the now-unused `.results-grid` CSS rule was removed rather than left dead. Not meaningfully testable without a real browser (jsdom doesn't do real box layout, so a DOM test would pass whether or not the buttons were actually visible) -- see the new Candidate management guardrails entry above for what to check by hand.
