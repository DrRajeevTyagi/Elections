...
35. **Activity Log, by election name.** Open the **Activity Log** tab. Both elections you just ran show up as named, clickable entries under "Elections" — click **House 3 try** (or whatever you named it) and the log narrows immediately to just that election's actions, no dropdown or Search press needed. Click **All Elections** to go back to everything.
...
36. **Storage indicator.** Point at the green "Storage: OK" on the Dashboard once, so whoever is on duty would recognise the red warning banner if storage ever failed for real.
